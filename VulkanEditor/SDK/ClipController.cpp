...
void ClipController::lerpUpdate(float dt, glm::vec3& pos, glm::vec3& rot, glm::vec3& scale)
{
	float timeStep = dt;
	timeStep *= mSlowMoMultiplier;
	timeStep *= mPlaybackDirection;	//this is why he suggusted -1 0 1
	...
	Keyframe tempNextKeyframe = mpClipPool->getClips().at(mClipIndex).mKeyframePool->getKeyframe(nextKeyframeIndex);
