...
void main()
{
	...
	float diff = max(dot(norm, lightDir), 0.0) * diffuseStrength;		//darker diffuse component the greater the angle, max is to make sure its not negative
	...
	outColor = (ambient + diffuse + specular) * texture(texSampler, fragTextureCoord);	
}