...
void main()
{
	...
	vec3 diffuse = (diff * lightColor);	//remember that we diffuse to the color of incoming light
	...
	vec3 result = (ambient + diffuse + specular) * texture(texSampler, fragTextureCoord).xyz; //* texture(texSampler, fragTextureCoord);	
